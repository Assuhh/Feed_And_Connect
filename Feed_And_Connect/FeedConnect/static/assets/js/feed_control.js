...
function showSection(sectionId) {
    ...
    // Load control section content dynamically
    if (sectionId === 'control') {
        fetch('/control-content/')
            .then(response => response.text())
            .then(html => {
                section.innerHTML = html;

                // Load the control JS after injecting HTML
                const script = document.createElement('script');
                script.src = "{% static 'js/feed_control.js' %}";
                script.defer = true;
                document.body.appendChild(script);
            })
            .catch(err => {
                section.innerHTML = `<div class="alert alert-danger">Failed to load control section. ${err}</div>`;
            });
    }
}
